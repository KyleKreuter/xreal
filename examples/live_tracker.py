#!/usr/bin/env python3
"""Live console head tracker built on the xreal lib.

    python3 examples/live_tracker.py

Keys: [space] zero yaw   [r] reset   [q] quit
"""
import sys
import time
import select
...
from xreal import XrealOne, HeadTracker, DeviceError
...
def main():
    ...
    try:
        ...
        print("NO-GO:", e)
        return
    print("GO. Madgwick 6-axis + ZUPT.  Keys: [space] zero  [r] reset  [q] quit\n")

    n = 0
    t0 = time.monotonic()
    try:
        with raw_keyboard() as key:
            for ori in tracker.stream(dev):
                k = key()
                if k in ("q", "\x03"):
                    break
                elif k == " ":
                    tracker.zero_yaw(ori)
                    print("\n[yaw zeroed]")
                elif k == "r":
                    tracker.reset()
                    print("\n[reset]")
